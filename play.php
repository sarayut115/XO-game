...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM moves WHERE game_id = ? AND move_row = ? AND move_col = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iii", $game_id, $row, $col);
    $stmt->execute();
    $move_exists = $stmt->get_result()->num_rows > 0;
...
function checkWinner($board, $size)
{
    ...
    if ($size >= 4) {
        ...
        for ($i = 0; $i < $size; $i++) {
            for ($j = 0; $j <= $size - 4; $j++) {
                $pattern = array_slice($board[$i], $j, 4);
        ...
        for ($i = 0; $i < $size; $i++) {
            for ($j = 0; $j <= $size - 4; $j++) {
                $pattern = array_column(array_slice($board, $j, 4), $i);
                if (count(array_unique($pattern)) === 1 && $pattern[0] != '') {
                    return $pattern[0];
                }
            }
...
<body>
    ...
    <?php if ($game["winner"]) : ?>
        ...
        <form method="post" action="select2Player.php" class="play-again-form">
